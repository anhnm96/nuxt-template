...
**The container's scroll anchoring is suspended for the length of a drag.** A gap appearing or retiring changes how tall the list is; where the container is scrolled to its end, the browser answers that by scrolling to hold its anchor still, which slides every row across a resting cursor. The rows a landing spot is read from then move with no cursor movement behind them, the reading changes, the gap goes, the height changes back, and the browser scrolls back. A drag list therefore turns anchoring off on the nearest ancestor that scrolls it when a drag involving it starts, and puts the property back the way it found it when the drag ends — outside a drag, anchoring is the container's business, being what holds a reader's place when something above them loads in.

This is what the list's own `overflow-anchor: none` was reaching for and could not do: on the list it only bars the list from being picked as the anchor, which leaves the container free to pick another and scroll for it just the same. The property has to sit on the element that scrolls, and that element belongs to the consumer.

**The auto-scroll forgets its measured limit when a drag ends, not when the cursor leaves the edge.** Those are different moments: the cursor leaving the edge band and coming back is one drag, and forgetting there hands the scroll a fresh measurement to trust, taken as likely as not over rows still in flight — which is the room the limit exists to leave alone.

**Rejected: refusing to read a landing spot from a row that is mid-move.** A row's box mid-flight is between two slots, so the reading is wrong in principle, and guarding it does fix the loop. It also freezes the preview for the length of every transition: moving the gap past a row sets that row animating, and that row is the one under the cursor, so the preview would stop following at exactly the moment the user is moving. The list has a test stating this, and it is the better call — the mid-flight reading converges on its own once nothing is scrolling underneath it.
...
**The drag list, mounted.** The existing seam for row moves: a list mounted with a fixed window and per-row boxes stubbed, with the animation API stubbed so that "was a move played" is a question the test can ask. Prior art: the block of tests that already distinguishes a reorder from a scroll step. Cases: an update that hands over the same window again plays nothing; a drag of the list's own stops the container anchoring and hands it back on `dragend`; an item arriving from elsewhere does the same; and a list unmounted mid-drag hands the container back on the way out, since nothing else would be left to.
...
**The anchoring helper, called directly.** New seam, for the one piece of this that touches an element the components do not own. A list in a container whose `overflow-y` is set inline, which is all the walk up to the scrolling ancestor reads. Cases: it puts back the value it found, including one the container set for itself; several lists sharing a container hold it until the last of them lets go; a hold released twice still counts once; and a list nothing scrolls falls back to the root, which is what anchors a viewport scroll.

**One Playwright test, against the drag demo page.** New seam, and the only one with the fidelity to catch this bug class: the loop needs real layout, real transforms counting towards the scrollable area, real animation timing, and real native drag events. Both regressions in this work passed the whole unit suite. The test drives two native drags with mouse press, move and release — the browser only starts a drag session for real input, so the higher level drag helper is not the tool. The order of the two is the requirement, not an implementation detail: a first drag lands an item at the end of a list already scrolled there, which is what arms the loop; only then does the second drag — the last row, slowly, to the middle of the row above it — have anything to catch. A test that skipped the first drag would pass whether the fix was in or not, since the second drag alone is steady either way. Between the two, the test samples the scroll position and the gap on every frame and asserts three things: that a gap appeared at all — without one the rest proves nothing, since a drag that never started would pass silently — that the container took exactly one scroll position throughout, and that the preview opened once instead of flickering. Prior art: the project's Playwright configuration and end-to-end directory, which held scaffolding only, so this is the first test of the drag components at that level. It needs the app running on the host named in the Playwright config.
...
- Compensating the scroll position ourselves when the gap changes the layout: moving the scroll by the gap's height to hold the rows under the cursor still is scroll anchoring by hand, it fights the virtualizer, and it changes what every consumer sees when a gap appears. Turning the browser's own anchoring off for the length of a drag is a different thing and is in scope, see the decision above — this line read as ruling that out too until the anchoring bug was found.
...
What made the diagnosis possible was recording, during a real drag, every drag event with its cursor position and every change of scroll position, tagged with whether the page had asked for it. A scroll with no request beside it is the browser's doing; a request with no movement after it is the browser refusing. Both readings were decisive, and neither is available from a simulated drag. It is a composable in the repo now rather than something to write again under pressure. It sleeps until a page is opened with `?dnd-record`, because it wraps `scrollTop` to see who wrote it and a tool that alters what it measures has no business running in every session; armed, it keeps one labelled log across every container watched, so several lists in one scroller read in the order things happened. Reproduce, then read it out of the console. Development only: `import.meta.dev`/`import.meta.server` keep every bit of its wiring — the property override, the listeners, the log itself — from ever running outside a dev server, which is the guarantee that matters; whether the unreachable branch is also stripped from the built file is a minifier's business, not this composable's to promise.

The second report against this area came from a list with no auto-scroll at all, and the loop looked identical: aim at the middle of the row above the one being dragged, and every row above jumps. It took both the browser's anchoring scroll and the move animation to produce — with anchoring suppressed it is steady, and with the moves made instant it settles after a single jump. Neither half is enough alone, which is why a reproduction has to be measured rather than reasoned about: the recorder's reading that no JavaScript had written `scrollTop` is what pointed at the browser in the first place.
