...
const props = defineProps<{ resolve: (v: boolean) => void }>()

async function handleClose(value: boolean, callback: (state: boolean) => void) {
...
<template>
  <DialogTemplate v-slot="{ setOpen }" :open="true">
    <div class="min-h-screen flex items-end justify-center px-4 pb-20 pt-4 text-center sm:block sm:p-0">
      <DialogPanel class="inline-block transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left align-bottom shadow-xl transition-all sm:my-8 sm:max-w-lg sm:w-full sm:p-6 sm:align-middle">
        ...
        <div class="sm:flex sm:items-start">
          ...
          <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
            <h3 id="modal-title" class="text-lg text-gray-900 font-medium leading-6">
              Deactivate account
            </h3>
            <div class="mt-2">
        ...
        <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
          <button
            ...
            @click="handleClose(true, setOpen)"
          >
            Deactivate
          </button>
          <button
            ...
            @click="handleClose(false, setOpen)"
          >
            Cancel
          </button>
        </div>
