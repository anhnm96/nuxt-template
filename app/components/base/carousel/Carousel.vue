...
function onScrollFinished() {
  ...
  scrollTimeout = setTimeout(() => {
    const newIndex = Math.round(elRef.value!.scrollLeft / (itemWidth.value + itemGap.value))
...
const itemsToShow = computed(() => {
  ...
  return Math.round((usesableWidth + itemGap.value) / (itemWidth.value + itemGap.value))
})
...
function refresh() {
  ...
  itemGap.value = parseFloat(getComputedStyle(elRef.value!).gap)
  // make sure we don't over translateX
